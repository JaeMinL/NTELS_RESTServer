...
#define RSVERR_THRD_PER_CONN_ONLY     116
...
#define RSV_INIT_GEN_CFG(_cfg, _port){\
    (_cfg)->svrCfg.port = (_port);\
    (_cfg)->svrCfg.thrdCnt = 0;\
    (_cfg)->svrCfg.sslCfg.cert = NULL;\
    (_cfg)->svrCfg.sslCfg.key = NULL;\
}

#define RSV_REG_STRM_USRARG(_sesCb, _strmUsrArg){\
    (_sesCb)->strmUsrArg = (_strmUsrArg);\
}
